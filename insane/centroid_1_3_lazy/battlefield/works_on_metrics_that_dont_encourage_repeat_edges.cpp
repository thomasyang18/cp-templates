/*
i.e. works on max(-dist -a[i]) for example, since we do not want to repeat edges
*/
...
namespace lca {
    ...
    void lcadfs(ll i, ll p, ll pw) {
        ...
        dep[i] = dep[p] + 1;
        par[i][0] = p;
        distFromRoot[i] = - pw; // WARN NEGATIVE DISTANCE CHAGNE LATER 
        tin[i] = time++;
        F(l, 1, L) {
...
struct centroid_tree {
    ...
    centroid_tree(vector<pl> edges, 
        ...
        auto do_distances = [&](){
            ...
            auto dfs = [&](auto &&self, ll i, ll p, ll d) -> void {
                ...
                for (ll x: adj[i]) if (x-p) self(self, x, i, d - 1); // negative distances make repeating edges bad 
    ...
    ll query(ll node_index) {
        ...
        if (node_index == centroid) {
            ll res = c[0] ? max(c[0]->evaluate(), c[1]->evaluate()) : evaluate(); // If i'm a leaf, just return my evaluated value
