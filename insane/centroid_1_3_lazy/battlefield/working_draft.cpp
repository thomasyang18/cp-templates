...
struct centroid_tree {
    ...
    centroid_tree(vector<pl> edges, 
        ...
        vector<pl> usedValues;

        for (auto &[x, _]: adj) {
            times.K = min(times.K, tin[x]);
            times.V = max(times.V, tout[x]);
            usedValues.emplace_back(x, 0);
            care.insert(x);
        }

        seg = lztree(move(usedValues));
    ...
    ll query(ll node_index) {
        push();
        // cout << "QUERYING "  << centroid << ", " << node_index << " " << n << endl;
        // cout << "wth? " << evaluate() << endl;
        // for (auto x: care) cout << x << " "; cout << endl;
