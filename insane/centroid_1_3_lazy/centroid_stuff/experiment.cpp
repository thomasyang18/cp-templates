// what leaf should be:
// leaf should be both a source of truth for its parents (lztree etc.)
// but also once u hit the end, u can just query for all nodes in its subtree. 
// so it needs both lztree and a naive arr. 
...
struct centroid_tree {
    ...
    constexpr static ll THRESHOLD = 10; // threshold for just storing repr as array 
